...
function runMatchingLogic(userInput, standalone) {
  ...
  pointerToCollectionToUse = autoSuggestList;
...
function setSmartInputData() {
  if (siw) {
    ...
    if (userInput && (userInput != "") && (userInput != '"') && (userInput.length > 2)) {
...
function smartInputWindow () {
  ...
  this.floater = document.getElementById(floaterValue);
  this.floaterContent = document.getElementById(floaterContentValue);
...
function initsmartInputWindowVlaue(floater,floaterContent){
 floaterValue=floater;
 floaterContentValue=floaterContent;

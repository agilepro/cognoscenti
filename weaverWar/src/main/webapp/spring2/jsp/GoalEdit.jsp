...
app.controller('myCtrl', function($scope, $http, $modal, AllPeople) {
    ...
    $scope.makeState = function(rec, newState) {
        if (!$scope.canUpdate) {
            alert("Unable to update action item because you are not playing an update role in this workspace");
            return;
        }
        var newRec = {};
        newRec.id = rec.id;
    ...
    $scope.toggleCheckItem = function(changeIndex) {
        if (!$scope.canUpdate) {
            alert("Unable to update action items because you are not playing an update role in the workspace");
            return;
        }

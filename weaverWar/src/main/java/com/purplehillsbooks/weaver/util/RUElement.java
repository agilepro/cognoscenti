...
public final class RUElement {
     ...
     *
     * <p>This will scan the vector to see if the given element is in it. If so, it updates the
     * timestamp to the passed in current time, -- and the name to a possibly new name. If not, it
     * adds it to the vector. If the total number of entries is greater than max, then it finds the
     * oldest entry, and kicks it out.
     *
     * <p>Takes the current time as a variable so that an entire interaction can be at a specific
     * time, not just the time that it happens to hit the routine at. This avoids possible paradoxes
     * when things that happened during one transaction have slightly different times.
     */
    public static void addToRUVector(
            List<RUElement> v, RUElement newElement, long currentTime, int max) {

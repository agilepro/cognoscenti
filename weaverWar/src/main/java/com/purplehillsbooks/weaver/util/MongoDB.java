...
public class MongoDB {
    ...
    /**
     * simple default query that gets all the records up to a limit probably only useful for
     * testing/debugging
     */
    ...
    public JSONArray querySortRecords(JSONObject query, JSONObject sort, int skip, int batchSize)
            throws Exception {
        try {
            ...
            try {
                ...
                if (count > 0) {
                    System.out.println(
                            "MONGO: " + count + " records (" + ms + "ms) from: " + queryString);
                }
                return ja;
            } finally {
                cursor.close();

 ...
 * Three way merge.
 *
 * <p>cur - this is the current official record with all changes from everyone old - this is what
 * this change assumed the current record to be new - this is what this change wants to change to
 *
 * <p>Changes that are identified in the current version completely missing from old and new are
 * preserved as these seem to have been made after the edit started and they simply did not know
 * about it.
 *
 * <p>But if in all other cases whatever is in the current version is replaced with whatever is in
 * the new version.
 */
public class ThreeWayMerge {
    ...
    /**
     * This is the easy part of the algorithm, which is walk through the strings advancing each as
     * long as they are all three exactly the same. Advance increments the pos pointer in the
     * PosString with the understanding that what is before the pos pointer has been found to be
     * identical. Remember that due to previous differences the actual offset might be different,
     * but we are walking through spans of the strings that are identical looking for the beginning
    ...
    /**
     * The goal here is to find the span of differences in the three strings. That is, we want to
     * find the next span of three or more characters that are the same in all three string after
     * the current point. We hope to find the shortest span across the three. In many cases, two
     * strings might be identical. But we need to find a span that exists in all three strings, and
     * that span that is closest to the beginning so that we have a minimal span of difference.
     *
     * <p>for example
     *
     * <p>abc ^ def ^ ghi abc ^ 123456 ^ ghi abc ^ x ^ ghi
     *
     * <p>In these three, the "ghi" terminates the difference span in all three strings, and the
     * difference span is 3, 6, and 1 character long respectively.
     *
     * <p>The problem is that you may find a match that is a long ways away, when a closer match
     * would do. We want to avoid this problem:
     *
     * <p>abc ^ def ^ ghi456ghi abc ^ 123ghi456 ^ ghi abc ^ x ^ ghi456ghi
     *
     * <p>So, we start looking for chunks (3-letter-strings) that are closest to the beginning of
     * each of the strings -- actually closest to the beginning of the the span after the section
     * that was found identical. Gradually look for further and further chunks. Continue to try to
    ...
    private void resolveDifferenceSpan() {
        ...
        if (curDiff.length() == 0 && oldDiff.length() == 0 && newDiff.length() == 0) {
            throw new RuntimeException(
                    "STRANGE, got zero zero zero for diff results ... they are all equal?");
        }
    ...
    public static void testOneMerge(String result, String curStr, String oldStr, String neuStr)
            throws Exception {

...
/**
 * NGPage is an abstract parent of a NGWorkspace. A Workspace is the main container of a
 * workspace/circle/etc. "Page" is the old term, "Leaf" is also an old term, avoid these. The
 * current term for the user interaction is "Workspace" This should all be moved into NGWorkspace to
 * simplify.
 */
public abstract class NGPage extends ContainerCommon {
    ...
    /**
     * To an existing workspace, add all the (1) Action Items (2) Roles of an existing workspace.
     *
     ...
     */
    public void injectTemplate(AuthRequest ar, NGPage template) throws Exception {
    ...
    /**
     * Clears the current page from the cache. This is the global "rollback" function. It does not
     * undo any changes but if this is called before discarding the reference to a page object, and
     * without saving that page, then the next access to the page will be from the prior saved
     * version of the page. This method should be called whenever an exception is caught (at the
     * root level) so that any possible changes during processing before the exception is thrown
     * away.
     */
    ...
    public void saveWithoutMarkingModified(String modUser, String comment, Cognoscenti cog)
            throws Exception {
    ...
    /**
     * This should be called everytime the page contents are changed in a way that might effect the
     * links on the page.
     */
    ...
    /**
     * This will mark the page as deleted at the time of the request and by the person in the
     * request.
     */
    ...
    /**
     * This will unmark the page from being deleted, clearing any earlier setting that the page was
     * deleted
     */
    ...
    /**
     * The AuthRequest object is needed to record the user and time of modification. In the
     * situation that a required section is missing (such as the Tasks section which is supposed to
     * be created at the moment the page is created, but there are some old pages today without
     * Tasks sections) then it does not really matter to record who added this required section. In
     * that case, pass a null in for the AuthRecord, and nothing will be recorded.
     */
    ...
    /**
     * This is the unique ID of the entire workspace across all sites. It is up to the system to
     * make sure this is created and maintained unique and it must never be changed (or links will
     * be broken). Linking should be by name if possible.
     */
    @Override
    ...
    public NGBook getSite() {
        if (prjSite == null) {
            ...
            throw new RuntimeException(
                    "Program Logic Error: something is wrong with NGPage object which has a null site ... this should never happen.");
        }
        return prjSite;
    }
    ...
    public void setSite(NGBook ngb) {
        if (ngb == null) {
            throw new RuntimeException(
                    "setSite called with null parameter.   Should not be using the 'default site' concept any more, this exception is checking to see if it ever happens");
        }
        ...
        if (lr instanceof LicenseForUser) {
            ...
            for (GoalRecord goal : this.getAllGoals()) {
                if (!goal.isPassive() && goal.isAssignee(ale)) {
                    // passive action items from other workspace should only effect those other
                    // workspaces,
    ...
    public GoalRecord createGoal(String requesterId) throws Exception {
    ...
    /**
     * implemented special functionality for workspaces ... there are site executives, and there are
    ...
    public boolean primaryOrSecondaryPermission(UserRef user) {
        ...
        for (GoalRecord gr : getAllGoals()) {
            if (gr.isPassive()) {
                // ignore any passive action items that are from other workspaces.  Only consider
                // local
                // goals
                continue;
            }
    ...
    public void deleteRoleRequest(String requestId) throws Exception {
        DOMFace roleRequests = pageInfo.requireChild("Role-Requests", DOMFace.class);
        List<Element> children =
                DOMUtils.getNamedChildrenVector(roleRequests.getElement(), "requests");
    ...
    /**
     * Find all the tasks on this page, and assign them new, arbitrary rank values such that they
     * remain in the same order. Assigned new values increasing by 10 so that other tasks can be
     * place between existing tasks if necessary.
     */
    ...
    public MeetingRecord createMeeting() throws Exception {
        DOMFace meetings = requireChild("meetings", DOMFace.class);
        MeetingRecord mr =
                meetings.createChildWithID(
                        "meeting", MeetingRecord.class, "id", this.getUniqueOnPage());
        mr.setState(1);
        return mr;
    }
    ...
    public DecisionRecord createDecision() throws Exception {
        ...
        DecisionRecord dr =
                decisions.createChildWithID(
                        "decision", DecisionRecord.class, "num", Integer.toString(max + 1));
    ...
    public EmailGenerator createEmailGenerator() throws Exception {
        ...
        EmailGenerator egen =
                meetings.createChildWithID(
                        "emailGenerator", EmailGenerator.class, "id", this.getUniqueOnPage());
        return egen;
    }
    ...
    public List<RoleRequestRecord> getAllRoleRequest() throws Exception {
        ...
        List<RoleRequestRecord> children =
                rolelist.getChildren("requests", RoleRequestRecord.class);
    ...
    // Are they reused??
    public RoleRequestRecord getRoleRequestRecord(String roleName, String requestedBy)
            throws Exception {
        RoleRequestRecord requestRecord = null;
        long modifiedDate = 0;
        for (RoleRequestRecord roleRequestRecord : getAllRoleRequest()) {
    ...
    public RoleRequestRecord createRoleRequest(
            String roleName,
            String requestedBy,
            long modifiedDate,
            String modifiedBy,
        ...
        RoleRequestRecord newRoleRequest =
                rolelist.createChild("requests", RoleRequestRecord.class);

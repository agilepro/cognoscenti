...
/**
 * The three classes: NGWorkspace, NGBook, and UserPage are all DOMFile classes, and there are some
 * methods that they can easily share. This class is an abstract base class so that these classes
 * can easily share a few methods.
 */
public abstract class ContainerCommon extends NGContainer {
    ...
    /**
     * Here is how schema version works. Each major file object will declare what the current schema
     * version and set it here for saving in the file. A file is ALWAYS written at the current
     * schema version.
     *
     * <p>Every time a file is read, all the schema upgrades are applied on reading, generally in
     * the constructor. So the version of the file IN MEMORY is always the latest schema, so you can
     * always write out the memory and always the latest schema.
     *
     * <p>But ... lots of files a read but not written. So the file on disk might be many schema
     * versions behind.
     *
     * <p>Before saving, if the schema had been out of date, then a pass is made to touch all the
     * sub-objects, and make sure that all objects are at the current schema level. Then the file is
     * written.
     *
     * <p>RULE FOR MANAGING VERSION Every time any schema change is that requires schema update, the
     * whole file schema versions is incremented. You must make sure that the schema update method
     * touches that component (with a constructor so that it is certain to be upgraded. The we are
     * assured that the entire file is current and can be written.
     *
     * <p>EXAMPLE You have a file with schema version 68. You make a change in the CommentRecord
     * that requires a schema migration. (1) You bump the schema version to 69. (2) You make a
     * comment that the CommentRecord change is what caused increment to 69. (3) You make sure that
     * the schemaUpgrade method touches (and upgrades) all comments.
     *
     * <p>That is it. The reason for making the comment is because in the future, we might be able
     ...
     *
     * <p>The next schema change, anywhere in the code, will bump the schema version to 70 and the
    ...
    /**
     * Only set schema version AFTER schemaUpgrade has been called and you know that the schema is
    ...
    /**
     * The implementation of this must touch all objects in the file and make sure that the internal
     * structure is set to the latest schema.
     */
    public abstract void schemaUpgrade(int fromLevel, int toLevel) throws Exception;

    public abstract int currentSchemaVersion();
    ...
    public NGRole getRequiredRole(String symbol) throws Exception {
        ...
        if (role == null) {
            ...
            if ("Executives".equals(symbol)) {
                desc =
                        "The role 'Executives' contains a list of people who are assigned to the site "
                                + "as a whole, and are automatically members of every workspace in that site.  ";
            }
            if ("Owners".equals(symbol)) {
                desc =
                        "The role 'Owners' contains one or more people who take full responsibility "
     ...
     * page. Links to this page could include this magic number, and it will allow a person with
     * that email address access to this page. It will prove that they have been sent the magic
     * number, and proof that they received it, and therefor proof that they own the email address.
     *
     * <p>Pass the email address that you are sending to, and this will return the magic number.
     * When the user clicks on the link match the magic number in the link, with the magic number
     * generated here, to make sure they are not hacking their way in.
     *
     * <p>The URL that the user clisk on must have BOTH the email address and the magic number in
     * it, because they are precisely paired. Don't expect to get the email address from other part
     * of environment, because remember that email addresses are sometimes transformed through the
     * sending of the document.
     *
     * <p>The magic number is ephemeral. They need to last long enough for someone to receive the
     * email and click on the link, so they have to last for days at least, probably weeks. This
     * algorithm generates a magic number that will last permanently as long as the workspace lasts.
     * This is good because there is no real way to migrate to a new algorithm for generating the
     * magic numbers. But in practice this algorithm might be changed at any time, causing
     * discomfort to those users that just received links, but presumably they would soon receive
     * new links with the new numbers in them. There is no real requirement that the number last
     * *forever*.
     */
    ...
    public License addLicense(String id) throws Exception {
        LicenseRecord newLement =
                infoParent.createChildWithID("license", LicenseRecord.class, "id", id);
        return newLement;
    }

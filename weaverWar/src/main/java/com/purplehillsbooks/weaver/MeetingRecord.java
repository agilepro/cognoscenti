...
public class MeetingRecord extends DOMFace {
    ...
    /**
     * The owner is the actual user who created this meeting. Email will appear to be from this
     * user.
     */
    ...
    /**
     * Reminder time is the amount of time (in minutes) before the meeting to automatically send
     * everyone the reminder.
     */
    ...
    /**
     * If the meeting is in running state, and if the specified person is not in the attendee list,
     * this will add them.
     *
     * <p>Returns true if it actually made a change, false if not
     *
     * @param uRef
     */
    ...
    /**
     * NO LONGER a special meeting which is actually the container for backlog agenda items. Should
    ...
    /**
     * increments all the positions of the agenda items that are AT or greater than the specified
     * position. Leaves the positions of those less than the position alone.
     *
     * <p>Usage to place a new agenda item at the position of 5, then call openPosition(5) to make
     * an opening, then set the desired item to position 5, then call renumber items to close any
     * gaps left.
     */
    ...
    /**
     * Find all the agenda items that are linked to a particular document. Pass in the universal id
    ...
    /**
     * This is used to set particular aspects of the meeting time, without having to save (and
     * potentialy overwrite) another person's settings, allowing people to be settings times
     * simultaneously.
     *
     * <p>Send in
     *
     * <p>{ "action": "SetValue", "isCurrent": true/false, "time": 15432432534, "user":
     * "george.washington@whitehouse.gov", "value": 3 }
     *
     * <p>or
     *
     * <p>{ "action": "AddTime", "isCurrent": true/false, "time": 15432432534 } { "action":
     * "RemoveTime", "isCurrent": true/false, "time": 15432432534 } { "action": "ChangeTime",
     * "isCurrent": true/false, "time": 15432432534, "newTime": 15432888888 }
     */
    ...
    public MeetingProposeTime findOrCreateProposedTime(String fieldName, long timeValue)
            throws Exception {
    ...
    private MeetingProposeTime removeUserFromAllSlots(String fieldName, String userName)
            throws Exception {
        List<MeetingProposeTime> timeSlot = getChildren(fieldName, MeetingProposeTime.class);
        for (MeetingProposeTime oneSlot : timeSlot) {
     ...
     * @throws Exception
     */
    public JSONObject getFullJSON(AuthRequest ar, NGWorkspace ngw, boolean allcomments)
            throws Exception {
        ...
        meetingInfo.put(
                "participants",
                AddressListEntry.getJSONArrayFromIds(this.getVector("participants")));
    ...
    public void updateFromJSON(JSONObject input, AuthRequest ar) throws Exception {
        ...
        if (input.has("participants")) {
            this.setParticipants(
                    AddressListEntry.uidListfromJSONArray(input.getJSONArray("participants")));
        }
    ...
    /**
     * This will update the existing agenda items with values from the JSONArray. This will NOT
     * remove any agenda items. If the id is "~new~" if will create an agenda item. This is most
     * useful to change the order of the items.
     */
    public void updateAgendaFromJSON(JSONObject input, AuthRequest ar, NGWorkspace ngw)
            throws Exception {
    ...
    /**
     * This takes a meeting JSONObject, the agenda portion and it create new cloned agenda items for
     * each in the array.
     */
    public void createAgendaFromJSON(JSONObject input, AuthRequest ar, NGWorkspace ngw)
            throws Exception {
        JSONArray agenda = input.optJSONArray("agenda");
        if (agenda == null) {
    ...
    public String getNameAndDate(Calendar cal) throws Exception {
        return getName()
                + " @ "
                + formatDate(getStartTime(), cal, "HH:mm z 'on' dd-MMM-yyyy", "(to be determined)");
    }
    ...
    public void sendReminderEmail(AuthRequest ar, NGWorkspace ngw, EmailSender mailFile)
            throws Exception {
        try {
    ...
    /**
     * This is a pain getting this right. The meeting might be scheduled in the future or in the
     * past. (the latter is not likely but possible).
     *
     * <p>You can set different settings for how much before the meeting to send the announcement.
     * Even if the meeting is in the future, the announcement time can be in the future or in the
     * past.
     *
     * <p>Regardless of the announcement time, the announcement email might have already been sent.
     *
     * <p>The meeting time might have been changed, and in that case a new announcement should be
     * sent, regardless of whether sent previously or not.
     *
     * <p>Meeting might be in draft mode, and so don't send any announcement until it changes to
     * Plan mode. ONLY send if it Plan more. Don't send in running or completed.
     *
     * <p>Here are the facts we have to work with: meeting time, reminder minutes, state, and actual
     * send time.
     *
     * <p>The meeting time and reminder minutes combine to form reminderPlanTime. There is a
     * reminderSent which is the actual time sent.
     *
     * <p>When the meeting time changes, you should send the notification again, regardless of
     * whether sent or not, regardless of whether this is in the future or the past. When the
     * meeting time changes, clear the sent flag, and clear the sent time.
     *
     * <p>Then, every polling cycle, check whether it is time to send or not, depending on expected
     * send time and state. If planning and expected time is in past, and have not already been
     * sent, then send it.
     *
     * <p>When email sent, set the reminderSent time, so that it is sent only once. The only thing
     * that clears reminder sent is changing the meeting time.
     */
    private class MScheduledNotification implements ScheduledNotification {
        ...
        public void sendIt(AuthRequest ar, EmailSender mailFile) throws Exception {
            ...
            if (needsSendingBefore(ar.nowTime)) {
                System.out.println(
                        "STRANGE: the meeting was just sent, but it does not think so. SENDTIME: "
                                + SectionUtil.getNicePrintDate(futureTimeToSend())
                                + " and MEETTIME: "
                                + SectionUtil.getNicePrintDate(meet.getStartTime()));
            }
    ...
    public void streamICSFile(AuthRequest ar, Writer w, NGWorkspace ngw) throws Exception {
        ...
        w.write(
                "DTEND:"
                        + getSpecialICSFormat(getStartTime() + (getDuration() * 60 * 1000))
                        + "\n");
        w.write("SUMMARY:" + getName() + "\n");
        String descriptionHtml =
                "This appointment is for a Meeting organized in Weaver.\n"
                        ...
                        + "\nWorkspace: "
                        + ar.baseURL
                        + ar.getResourceURL(ngw, "FrontPage.htm");
    ...
    /**
     * This produces a list of meeting attendees, but ONLY those who are members of the site in the
     * list of site users. Ad-hoc users added to a meeting are ignored.
     */

...
public class PageInfoRecord extends DOMFace {
    ...
    public PageInfoRecord(Document nDoc, Element nEle, DOMFace p) throws Exception {
        super(nDoc, nEle, p);
    ...
    /**
     * Marking a workspace as deleted means that we SET the deleted time. If there is no deleted
     * time, then it is not deleted. A workspace that is deleted remains in the archive until a
     * later date, when garbage has been collected. If the workspace has remained deleted for more
     * than 90 days, then the file is actually removed from the file system (and so are all the
     * attachments)
     */
     ...
     * most cases. Set the date to zero in order to clear the deleted flag and make the workspace to
     * be not-deleted
     */
    ...
    /**
     * Marking a workspace as frozen means that we SET the frozen time and user. If there is no
     * frozen time, then it is not frozen.
     *
     * <p>The administrator of a workspace is allowed to freeze or unfreeze it. It is a lot like
     * deleted, in that users can not make changes, a frozen workspace will remain in that state
    ...
    public void unfreezeWorkspace() {
        if (isDeleted()) {
            throw ProgramLogicError.newBasic(
                    "attempt to unfreeze a workspace which is deleted.  Undelete the workspace first.");
        }

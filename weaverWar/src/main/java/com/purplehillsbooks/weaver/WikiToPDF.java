...
/**
 * Takes the Wiki formated text, and produces PDF output. NOTE: this is NOT muti-thread safe! Use on
 * only one thread at a time. Usage pattern:
 *
...
public class WikiToPDF {
    ...
    public static final Color paleYellow = new Color(252, 255, 226);
    public static final Color lightBlue = new Color(178, 229, 255);
    ...
    static final int TOP_MARGIN = 725; // Absolute top of text
    static final int BOTTOM_MARGIN = 60; // Never print below this
    static final int LEFT_MARGIN = 75; // Absolute left of text
    static final int RIGHT_MARGIN = 550; // Absolute right of text
    ...
    protected int currentLineSize =
            12; // used for calculating the first line position when a new page feed
    ...
    //
    // When a new paragraph starts, it moves the trailing, then preceeding space of the next
    // paragraph
    ...
    /**
     * This is a hack due to code transition. decodes the request parameters and then causes the PDF
     * generation try not to use this if possible.
     */
    ...
    private void writeTOCPage(
            Frame docInterior, List<TopicRecord> memberNoteList, List<MeetingRecord> meetings)
            throws Exception {
        ...
        if (memberNoteList.size() > 0) {
            ...
            for (TopicRecord note : memberNoteList) {
                ...
                setPFont();
                writeLineNoSpacing(
                        tocFrame, "   " + Integer.toString(noteCount) + ".  " + note.getSubject());
            }
        }
        noteCount = 0;
        if (meetings.size() > 0) {
            ...
            for (MeetingRecord meet : meetings) {
                ...
                indent = 15;
                writeLineNoSpacing(
                        tocFrame, "   " + Integer.toString(noteCount) + ".  " + meet.getName());
            }
        }
        indent = 0;
     ...
     */
    private void writeNoteToPDF(Frame fullDocContents, TopicRecord note, int noteNum)
            throws Exception {
        ...
        para.addTextCarefully(
                "Last modified by:  " + lastEditor.getName() + " on " + editTime,
                8,
                PDType1Font.HELVETICA);
    ...
    private void writeMeetingToPDF(Frame fullDocContents, MeetingRecord meet, int meetNum)
            throws Exception {
        ...
        para.addTextCarefully(
                "Last modified by:  " + lastEditor.getName() + " on " + startTime,
                8,
                PDType1Font.HELVETICA);
        ...
        for (AgendaItem ai : meet.getAgendaItems()) {
            ...
            para.addTextCarefully(
                    Integer.toString(agendaNum) + ": " + ai.getSubject(),
                    16,
                    PDType1Font.HELVETICA);
    ...
    private void writeComment(Frame frame, CommentRecord cr) throws Exception {
        ...
        writeLineNoSpacing(
                firstLineShaded, dateStr + "  (" + commentType + ")  " + cr.getUser().getName());
    ...
    private Frame startNamedSection(Frame entireDocContents, String title) throws Exception {
        ...
        writeLineNoSpacing(
                titleBoxFrame,
                "Site / Workspace: " + book.getFullName() + " / " + ngw.getFullName());
    ...
    private void writeDecisionsToPDF(Frame fullDocContents) throws Exception {
        ...
        for (DecisionRecord dr : ngw.getDecisions()) {
            ...
            writeLineNoSpacing(
                    innerFrame,
                    "Decision #" + dr.getNumber() + " - " + convertDate(dr.getTimestamp()));
    ...
    /**
     * eliminate any chars > 128 because PDFBox can't handle them right now also eliminate double
     * white space characters to simulate HTML
     */
    ...
    protected Paragraph startHeader(Frame frame, String line, int level) throws Exception {
        terminate();
        Paragraph para = frame.getNewParagraph();
        indent = 0;
        majorState = HEADER;
    ...
    protected void scanForStyle(Paragraph para, String line, int scanStart) throws Exception {
        int pos = scanStart;
    ...
    public void outputProperLink(Paragraph para, String linkURL) throws Exception {

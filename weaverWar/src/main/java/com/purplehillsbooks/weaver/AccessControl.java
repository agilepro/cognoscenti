...
public class AccessControl {
    ...
    /**
     * When the web request is trying to access a particular document, this will say whether that
     * document should be accessed, based on logged in user being a member of the workspace, or
     * whether a special session permission was granted.
     *
     * <p>For documents, the localIdMagicNumber must be in the "mndoc" parameter to get special
     * session status.
     *
     * <p>Side Effect: if the right magic number is found in URL, the session is marked and this
     * special access capability will persist as long as the session does.
     */
    ...
    public static void allowSpecialAccessDoc(
            AuthRequest ar, NGWorkspace ngw, AttachmentRecord attachRec) {
        String resourceId = "doc:" + attachRec.getId() + ":" + ngw.getKey();
        ar.setSpecialSessionAccess(resourceId);
    }
    ...
    public static boolean canAccessTopic(AuthRequest ar, NGWorkspace ngw, TopicRecord topicRec)
        ...
        if (!expectedMN.equals(mnnote)) {
            System.out.println(
                    "CAN-ACCESS-TOPIC: not allowed because ("
                            + mnnote
                            + ") is not ("
                            + expectedMN
                            + ")");
    ...
    public static void allowSpecialAccessTopic(
            AuthRequest ar, NGWorkspace ngw, TopicRecord topicRec) {
        String resourceId = "topic:" + topicRec.getId() + ":" + ngw.getKey();
        ar.setSpecialSessionAccess(resourceId);
    }
    ...
    public static boolean canAccessRoleRequest(
            AuthRequest ar, NGWorkspace ngw, RoleRequestRecord roleRequestRecord) throws Exception {
    ...
    public static String getAccessRoleRequestParams(
    ...
    public static boolean canAccessSiteRequest(
            AuthRequest ar, String userKey, SiteRequest accountDetails) throws Exception {
    ...
    public static String getAccessSiteRequestParams(String userKey, SiteRequest accountDetails)
            throws Exception {
    ...
    public static boolean canAccessMeeting(AuthRequest ar, NGWorkspace ngw, MeetingRecord meet)
            throws Exception {
        // then, if user is logged in, and is a member, then can access
        if (ar.isLoggedIn()) {
            UserProfile user = ar.getUserProfile();
    ...
    public static void allowSpecialAccessMeeting(
            AuthRequest ar, NGWorkspace ngw, MeetingRecord meet) {
        String resourceId = "meet:" + meet.getId() + ":" + ngw.getKey();
        ar.setSpecialSessionAccess(resourceId);
    }

 ...
 *
 * <p>There exists a collection of NGWorkspace objects (process leaves). This class helps to form an
 * index to those objects and to provide basic metadata about those pages without having to have all
 * pages (all leaves) in memory at the same time. The index holds the page name, page key,
 * references from that page, pages that refer to that page, which book, last change, whether a
 * request for membership is pending, and who the admins (authors) of the page are. These values are
 * selected in order to allow for quick searching and listing of exisitng pages according to certain
 * common criteria. Most importantly, this allows for back-links to be detected readily: since we
 * have a list of forward links, we can scan through there and build a list of back links.
 *
 * <p>Forward links are found by using a method on NGWorkspace to find all the outbound links. In
 * this case, a link is string using wiki-link rules (can be a name and link together separated by
 * vertical bar). This can also include external links, which are ignored by this index. Internal
 * links are simply the name of leaf being addressed. This name is case-insensitive and
 * punctuation-insensitive. This is accomplished by converting the name to a lowercase and stripping
 * all non alpha-num characters out. This simplified version of the name is compared between what
 * the page is linking to and the simplified names of the other pages.
 *
 * <p>Leaves (pages) can have multiple names, so that the name can be changed without breaking links
 * -- old reference will continue to work.
 *
 * <p>Names are not necessarily unique: there can be multiple leaves with the same name, because
 * users are not prevented from entering a particular name. This means that there needs to be a
 * "disambiguation" step in the case that there are other pages with the same name.
 *
 * <p>The approach is to have two objects: one object NGPageIndex represents the leaf (page) and the
 * other represents a particular name value (NGTerm). The term object represents a many-to-many
 * association between all the pages that link to a term, and all the pages that can be targets of
 * that term. Thus the NGPageIndex object has a "name" collection of terms, and a "links" collection
 * of terms. Each NGTerm has a "target pages" collection of index entries, and a "source links"
 * collection of index entries.
 *
 * <p>For example, a page "p1" may have three names: "a", "b", and "c". The page "p1" will be
 * represented by an NGPageIndex object. There will be three NGTerm objects, one for each name. Each
 * of those NGTerm objects will have a single target reference back to the page "p1". If a page "p2"
 * links to "a", then in the link collection for "p2" it will have a pointer to the term "a", and
 * term "a" will include "p2" in the "source link" collection.
 *
 * <p>If you are on page "p1", and want to find all the pages that point to "p1", then you find all
 * the terms that represent the names of "p1", and find all the pages that link to that term.
 *
 * <p>This has the effect of over counting in the case of non-unique names. If there are three pages
 * that have a single link to "a", and there are three pages that have the single name "a", then
 * each of the three target pages will show three pages linking to them. And each page will
 * effectively link to all three of the pages with the same name. Even though the page has only one
 * link. In practice, the user will have to choose the page when traversing the link.
 */
public class NGPageIndex {
    ...
    /**
     * containerType is the type of container whether it is Site, Workspace or User See constants
     * below
     */
    ...
    public static Hashtable<String, List<ArrayBlockingQueue<String>>> blqList =
            new Hashtable<String, List<ArrayBlockingQueue<String>>>();
    ...
    /**
     * Returns the collection of all target pages that are linked FROM this page. List contains
     * NGPageIndex objects. Remember, this may contain multiple pages for a single link if there are
    ...
    /**
     * Returns the collection of all source pages that link TO this page. List contains NGPageIndex
     * objects. Remember, this may contain multiple pages for a single link if there are multiple
    ...
    /**
     * Determine whether there are any inbound links, and if so it is not an orphan, but if there
     * are none, then it is an orphan.
     */
    ...
    /**
     * Given a string tag value, looks for and returns all the pages that are tagged with that tag.
     *
     * <p>Must NOT manipulate resulting vector in any way!
     */
    ...
    /**
     * Get the container object associated with this index entry, or return a null if one can not be
     * found.
     */
    ...
    /**
     * Get the container object associated with this index entry, or throw an exception if that
     * container can not be found. This method never returns a null;
     */
     ...
     *
     * <p>NGPageIndex is the master, which calls NGBook and NGWorkspace.
     */
    ...
    public static synchronized void initAllStaticVars() {
        blqSize = 10;
        ...
        try {
            ...
            if (wasLocked) {
                ...
                if (lockBlq.size() > 0) {
                    System.out.println(
                            "    LOCKTWIST: tid="
                                    + thisThread
                                    + " blocking queue is NOT EMPTY so not really locked!");
                }
            }
            ...
            if (lockObj == null) {
                ...
                System.out.println(
                        "    LOCKRCV: tid="
                                + thisThread
                                + " successfully got lock for spaceId="
                                + containerKey);
            }
            ...
            // lockedByAuditException = new Exception("    LOCKED: tid="+thisThread+"  spaceId=" +
            // containerKey);
    ...
    public void clearLock() {
        ...
        try {
            if (lockedBy != thisThread) {
                ...
                WeaverException.traceException(
                        ...
                        "    LOCKVICTIM: tid="
                                ...
                                + "ms ago!");
                if (lockAge < 10000) {
                    // don't understand how this lock got taken from this thread, but since it is
                    // young
            ...
            if (lockBlq.size() == 0) {
                // should be empty, but never add a second one to the queue, because then it could
                // end up
    ...
    /**
     * Here is the deal: when you access a workspace or other major object, it is locked to that
     * thread so that we don't have multiple threads working on the same data.
     *
     * <p>However, some routines walk through multiple major objects, and this might lead to them
     * holding lots and lots of locks, causing deadlock problems. Those routines want to unlock the
     * major objects.
     *
     * <p>Unlocking in a method is safe ONLY if you enter the routine without any locks. Nested
     * locks are not tracked, so you can recursively fetch a major object without locking yourself
     * out. But if one of those recursive fetches ALSO does an unlock, then you are unlocking for
     * the parent AS WELL, and that is mucho-dangerous.
     *
     * <p>The solution: every method that is going to walk through major objects, and unlock in the
     * middle of that, must ASSERT that there were no long being held by that thread before
     * entering. This is the only way to know that unlocking will be safe. The all calling methods
     * on the way must guarantee that they are not holding a lock whenever such a method is called.
     *
     * <p>This method asserts that no locks are held by the thread, and throws an exception if it
     * finds any locks held by this thread. That would be a no-no.
     */
    public static void assertNoLocksOnThread() throws Exception {
        ...
        if (lockMap.containsKey(ctid)) {
            ...
            Exception e =
                    new Exception(
                            "program logic error: Thread tid="
                                    + thisThread
                                    + " is holding a lock when it should not.  "
    ...
    /**
     * only Cognoscenti object should need to call the constructor, since these are created purely
     * by scanning files, or by adding containers. The index is built and maintained by Cognoscenti.
     */
    ...
    /**
     * The container (workspace or site) can have any number of names. For each name, an associated
     * term is found, and that term is made to point to this container.
     */
    ...
    /**
     * Containers have outbound links. This method finds all the links, and then finds the
     * associated terms, then marks those terms as having source links from this container.
     */
    ...
    /**
     * unlinkAll disconnects this NGPageIndex object from the terms so that it can be discarded.
     * This must be called when an index entry is removed from the index.
     */

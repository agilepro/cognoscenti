...
public class UserManager {
    ...
    private static Hashtable<String, UserProfile> userHashByKey =
            new Hashtable<String, UserProfile>();
     ...
     *
     * <p>Note -- All of the methods that read or write these variables should be synchronized
     * because the XML DOM is not entirely thread-safe. This user list tends to be accessed from
     * many requests. Also, this user list does not tend to talk to any other classes or do anything
    ...
    /**
     * Converting this from a pure static class to a singleton class that is constructed and managed
     * by the Cognoscenti class. The members will be static for a while, but the goal is to
    ...
    private void readJSONFile() throws Exception {
        ...
        for (int i = 0; i < users.length(); i++) {
            ...
            if (uid == null) {
                System.out.println(
                        "USER ELIMINATION: user ("
                                + up.getKey()
                                + ") does not have any universal id");
                continue;
            }
            ...
            if (other != null) {
                System.out.println(
                        "USER MANAGER FOUND duplicate user key to DELETE: "
                                + key
                                + ", "
                                + up.getUniversalId());
            ...
            for (String removableId : idsToRemove) {
                System.out.println(
                        "USER MANAGER REMOVING one global ID: "
                                + up.getKey()
                                + ", "
                                + up.getUniversalId()
                                + ", "
        ...
        for (UserProfile up : allUsers) {
            ...
            for (String idval : up.getAllIds()) {
                ...
                if (otherKey != null && !otherKey.equals(key)) {
                    ...
                    try {
                        System.out.println(
                                "USERS: two users claim the same email address (" + idval + ")");
    ...
    public static synchronized UserProfile getUserProfileByKey(String key) {
        if (key == null) {
            throw new RuntimeException(
                    "getUserProfileByKey requires a non-null key as a parameter");
        }
    ...
    public static synchronized UserProfile lookupUserByAnyId(String anyId) {
        ...
        if (anyId == null || anyId.length() == 0) {
            throw new RuntimeException(
                    "lookupUserByAnyId requires a non-null, non-empty anyId parameter");
        }
    ...
    /**
     * Read through the user profile file, find all the users that are disabled, and remove them
     * from the user profile list.
     */
    ...
    /**
     * Some parts of the program are given a list of email addresses or other id values. This will
     * take the list, parse it, and return a list of AddressListEntry objects.
     */
    ...
    /**
     * Given a list of AddressListObjects, this will make a nicely coma delimited list of the names
     * of those users.
     */

...
/**
 * NGCommonFile is a set of methods that are are needed on the three main file types: NGWorkspace,
 * NGBook, and UserPage
 */
public abstract class NGContainer extends DOMFile {
    ...
    /**
     * Every container has a key that uniquely identifies it from all other containers. In many
     * cases the key is used as the name of the file that stores it, or as a key value for a query
     * to retrieve all the data. The key is generated at the time of creation, and it remains a
     * constant, permanent, identified for this batch of information. The exact key structure is not
     * important, and may not be consistent over time or across different container types. It should
     * be treated as an obaque values. The only thing that matters is that it uniquely identifies
     * one container.
     */
    public abstract String getKey();

    /**
     * A role is a collection of user reference objects with a name, description and some other
     * metadata. Roles can exist on projects (NGWorkspace), sites (NGBook) or user profiles
     * (UserPage). In this last case, the role object holds what might better be called a
     * "Relationship".
     *
     ...
     * algorithm generates a magic number that will last permanently as long as the project lasts.
    ...
    /**
     * Returns a globally unique for the container (project/book/userpage) by combining the server's
     * universal id with the project key. Note: the server id is not guaranteed to remain constant
     * unless specified by the server administrator.
     */

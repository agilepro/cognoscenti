...
public class NGSession {
    ...
    /**
     * table of page ids that this session is an honorary member of because of using a special
     * license. This persists for the duration of the session.
     */
    ...
    /**
     * The proper way to get the NGSession instance that is assocaited with a given HttpSession
     * instance.
     */
    ...
    /**
     * Construct an NGSession object to hold information for this browser session.
     *
     * <p>Why not pass the http session to it and store a reference? The reason is that the
     * HttpSession object will be serialized out and then serialized back in again. Will the
     * reference be preserved through that? I don't think so.
     */
    private NGSession(HttpSession n_session) throws Exception {
        ...
        if (key != null) {
            ...
            UserProfile oldUser = UserManager.lookupUserByAnyId(key);
            if (oldUser != null) {
    ...
    public void addVisited(NGWorkspace ngw, long currentTime) {
        if (ngw == null) {
            throw ProgramLogicError.newBasic(
                    "addVisited was called with a null parameter.  That should not happen");
        }
    ...
    /**
     * Configuration values are cached, but this method will clear the cache and force the config to
     * be read from disk again. This can be called at any time, and only effect performance.
     */
    public void flushConfigCache() {}

    /**
     * pass in the simple file name of a file that exists in the server's local config file, and
     * this will return a File object with the full path to that file (whether it exists or not).
     */
    ...
    /**
     * Get rid of all marks in the session about this user or any rights or capabilities that the
     * usr might have, including: session.setAttribute("userKey", null);
     * session.removeAttribute("specialAccessUser");
     *
     * <p>This method gets rid of ALL attributes...
     */
    ...
    /**
     * Get the user profile of the currently logged in user (if there is any) or return null if not.
     */
    public UserProfile findLoginUserProfile() {
        ...
        UserManager.getStaticUserManager();
        return UserManager.lookupUserByAnyId(userGlobalId);
    }

    /**
     * make the proper markings in the session to remember who the logged in user is. This should
     * only be called when the user is POSITIVELY identified, such as after logging in.
     */
    ...
    /**
     * It is possible for a hacker to probe and get information from a server by observing what
     * makes an error and what does not. The dange of this is greatly reduced if errors a slow. If
     * an error takes a few seconds, then it takes too long to mine the errors for useful
     * information. If you are a legitimate user, then a delay of a few seconds does not matter.
     * ONLY, it is a much nicer user interface if the error response is immediate. The compromise is
     * this. The FIRST error will be fast, very fast. Subsequent errors -- within a give time span
     * -- should be slower. A simple rule: Never give another error message until 5 seconds after
     * the last. This will never slow the FIRST error, but only subsequent ones. Once you have gone
     * 5 seconds without an error, you are back to fast errors again.
     */

...
/**
 * This is the main class for the Cognoscenti object package. This is a singleton pattern, and holds
 * all the root configuration information. Stores itself as an attribute on the servlet context Use
 * this class for initialization and accessing the main index to objects within Cognoscenti
 */
public class Cognoscenti {
    ...
    /**
     * Starts the background task to initialize the server from config files. Also attempts to
     * initialize immediately. This should be called ONLY ONCE when the server starts the first
     * time. After that, methods on this can pause and restart the server.
     *
    ...
    /**
     * For most of the server functions, this is the method to test to see if the server is up,
     * running, and handling requests. When this is false, then only very special administrator
    ...
    /**
     * Call this in order to erase everything in memory and free up all of the cached values.
     * Returns the module to uninitialized state. You need to reinitialize after this. Useful before
     * calling garbage collect and reinitialize.
     */
     ...
     *
     * @param rootFolder is the root on the installed folder and requires that there be a file at
     *     {rootFolder}/WEB-INF/config.txt
     * @param backgroundTimer is used for all the background activity for sending and receiving
     *     email, passing a null in will disable email sending and receiving
    ...
    public synchronized void initializeAll(Timer backgroundTimer) throws Exception {
        ...
        try {
            ...
            if (isInitialized) {
                ...
                System.out.print(
                        "Reinitialization: changing from "
                                + theConfig.getFileFromRoot("")
                                + " to "
                                + rootFolder);
    ...
    /**
     * This method must not be called by any method that is used during the actual initialization
     * itself. It is designed to be called by threads that are NOT involved in initialization, in
     * order to cleanly wait for initialization to be completed. This method will wait for up to 20
     * seconds for initialization to complete with either a failure or a successful initialization.
     */
    ...
    /**
     * provides compatibility with earlier version that would attempt to initialize on any given
     * page refresh, and throw an exception if it failed. Newer approach is to initialize once, but
     * this will throw the exception anyway if one was found attempting to initialize.
     */
    ...
    /**
     * Combined key is "site|workspace" That is, the site key, a vertical bar, and the workspace key
     */
    ...
    /**
     * Note, the simply workspace key is NOT UNIQUE. This will work only as long as it is, otherwise
     * you might get the wrong key. This method is meant only as a temporary solution to allow
     * people with old lists of just the workspace, to migrate to the combined keys.
     */
    ...
    /**
     * Finding pages by name means that you might find more than one so you get a vector back, which
     * might be empty, it might have one or it might have more pages.
     */
    ...
    /**
     * This is a convenience function that looks a particular workspace up in the index, finds the
     * index entry, and then IF it is a workspace, returns that with the right type (NGWorkspace).
     * Fails if the key is not matched with anything, or if the key is for a site.
     */
    ...
    /**
     * This is a convenience function that looks a particular site up in the index, finds the index
     * entry, and then IF it is a site, returns that with the right type (NGBook) Fails if the key
     * is not matched with anything, or if the key is for a workspace.
     */
    ...
    /**
     * Returns a vector of NGPageIndex objects which represent workspaces which are all part of a
     * single site which are not deleted.
     */
    ...
    public List<NGPageIndex> getAllWorkspacesInSiteIncludeDeleted(String accountKey)
            throws Exception {
        List<NGPageIndex> ret = new ArrayList<NGPageIndex>();
        for (NGPageIndex ngpi : allContainers) {
            if (!ngpi.isWorkspace()) {
    ...
    /**
     * Sept 2017 change this algorithm. the sites must be children of the library file folders pased
     * in, and only direct children.
     *
     * <p>Then the workspaces can only be the direct chidlren of sites.
     *
     * <p>A workspace found in a site is automatically associated with that site.
     *
     * @param folder
     * @param allWorkspaces
     * @param allSites
    ...
    /**
     * When a site is physically removed from the disk, this method must be called to make it
     * disappear from the running index in memory.
     */
    ...
    /**
     * Get the first page that has email that still needs to be sent Returns null if there are not
     * any
     */
    ...
    /**
     * When you find that a workspace does not have any more email to send, call
     * removePageFromEmailToSend to remove from the cached list.
     */
    ...
    public synchronized List<SearchResultRecord> performSearch(
            AuthRequest ar, String queryStr, String relationship, String siteId, String workspaceId)
            throws Exception {
    ...
    public synchronized void recordVisit(
            String userKey, String site, String workspace, long timestamp) {
    ...
    /**
     * This returns the current time EXCEPT it guarantees that it never returns the same time twice,
     * incrementing the time if necessary by a few milliseconds to achieve this.
     */

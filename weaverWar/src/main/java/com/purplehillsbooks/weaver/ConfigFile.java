...
public class ConfigFile {
    ...
    /**
     * This is where the properties are cached internally in memory when the class is properly
     * initialized.
     */
    private Properties props = null;

    /**
     * Given the path to a file within the application, this will return the absolute File path
     * object to access it.
     */
    ...
    /** Given a name of a configuration file, this will return the File object to access it. */
    public File getFile(String fileName) {
        return new File(webInfPath, fileName);
    }

    /**
     * Initialize by passing in the root folder for the application. This is expected to have a
     * {root}/WEB-INF/config.txt file in there.
     *
    ...
    private void init(File newRoot) throws Exception {
        try {
            ...
            if (!configFile.exists()) {
                System.out.println(
                        "*** No configuration file found at: " + configFile.getAbsolutePath());
                ...
                if (exampleConfigFile.exists()) {
                    ...
                    System.out.println(
                            "*** config_example.txt copied to: " + configFile.getAbsolutePath());
                }
                ...
                if (!ssofiConfig.exists() && exampleSsofiConfig.exists()) {
                    ...
                    System.out.println(
                            "*** ssofi_example.config copied to: " + ssofiConfig.getAbsolutePath());
                }
                ...
                if (!emailConfig.exists() && exampleEmailConfig.exists()) {
                    ...
                    System.out.println(
                            "*** EmailNotification_example.properties copied to: "
                                    + emailConfig.getAbsolutePath());
                }
            }
    ...
    /**
     * Check whether this class has been initialized and can say anything about the configuration
     * parameters.
     */
    ...
    /**
     * getConfigProperties will return the configuration properties installed in WEB-INF/config.txt.
     * Complains if it can not find such a file. This is the static version. Will never return null.
     */
    ...
    /**
     * Returns an array of strings from a configuration settings which is SEMICOLON delimited. That
     * is, the config value is a list of values separated by semicolons. The values can not
     * themselves have semicolons in them. This method will split them and return a proper array.
     */
    ...
    /**
     * Checks that the supplied full path exists, and that it is a directory. Throws standard errors
     * if it is not.
     */
    ...
    /**
     * The config file has a setting for libFolder that can have multiple paths in it to folders
     * containing sites.
     *
     * <p>If the libe folder is specified and it does not exist, it is created
     *
     * <p>If it is not configured, then the parent data folder is found, and a subfolder named
     * "sites" is used
     *
    ...
    /**
     * Returns a user specified globally unique value, or else a random value if one was not set.
     * The purpose is to provide a unique jey for the server. If the user does not set a value, then
     * the unique key may be different every time the server is started. However, two servers are
     * highly unlikely to have the same value, and that is the purpose, to distinguish servers.
     *
     * <p>Use can specify the value in the config file setting: ServerId
     */

 ...
 *
 * <p>(1) generates an alphanumeric id based on timestamp
 *
 * <p>(2) generates a four digit timestamp unique within a set
 */
public class IdGenerator {
    static long lastKey = System.currentTimeMillis();

    /**
     * Generates a value based on the current time, but checking that it has not given out this
     * value before. If a key has already been given out for the current time, it increments by one.
     * This method works as long as on the average you get less than one ID per second.
     */
    ...
    /**
     * Get a four digit numeric id which is unique on the page. Pass in a vector containing all the
    ...
    /**
     * A double key is a combination of a timestamps (to guarantee uniqueness), and a random number
     * (to make it hard to guess). The random number is generated using a secure random number
     * generator, so that it is not predictable. To guess this key, you need to know two timestamp
     * values to the millisecond. While the current value might be guessed to be a timestamp around
     * the time that the request was made, The other is an indeterminate value...
     */
    private static SecureRandom rand = new SecureRandom();

    public static synchronized String generateDoubleKey() {

...
public class UserController extends BaseController {
    ...
    public void userHome(
            @PathVariable String userKey, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
    ...
    public void participantProjects(
            @PathVariable String userKey, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
    ...
    public void userMissingResponses(
            @PathVariable String userKey, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
    ...
    public void RemoteProfileUpdate(
    ...
    public void updateProfile(
            HttpServletRequest request, HttpServletResponse response, @PathVariable String userKey)
            throws Exception {
        AuthRequest ar = AuthRequest.getOrCreate(request, response);
        try {
    ...
    @RequestMapping(
            value = "/{siteId}/{pageId}/approveOrRejectRoleReqThroughMail.htm",
    ...
    public void gotoApproveOrRejectRoleReqPage(
        ...
        try {
            ...
            if (!canAccessPage) {
                showDisplayWarning(
                        ar,
                        "In order to see this section, you need to be logged in, and a member of the workspace.");
                return;
            }
    ...
    public void updateMicroProfile(HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        AuthRequest ar = AuthRequest.getOrCreate(request, response);
        ...
        try {
            ...
            if (ar.isLoggedIn()) {
                ...
                ar.resp.sendRedirect(
                        ar.baseURL + "v/" + profile.getKey() + "/NotificationSettings.htm");
                return;
            }
    ...
    public void searchPublicNotesJSON(
            @PathVariable String userKey, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        ...
        try {
            ...
            if (searchText.length() > 0) {
                searchResults =
                        ar.getCogInstance()
                                .performSearch(ar, searchText, searchProject, null, null);
            } else {
    ...
    */
        ...
        try {
            ...
            if (searchedFor == null) {
                if (userKey.indexOf("@") < 0) {
                    showDisplayWarning(
                            ar,
                            "Can't find any person with that key ("
                                    + userKey
                                    + ").  Maybe that person no longer has an account, or maybe there was some other mistake.");
                    return;
                }
    ...
    public void FindPerson(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (userKey == null || userKey.length() == 0) {
                showDisplayWarning(
                        ar,
                        "The FindPerson page requires an ID to search for but that appears to be missing from the URL.");
                return;
            }
            ar.req.setAttribute("userKey", userKey);
            UserProfile searchedFor = UserManager.lookupUserByAnyId(userKey);
    ...
    public void confirmEmailAddress(
        ...
        try {
            if (!ar.isLoggedIn()) {
                showDisplayWarning(
                        ar,
                        "Please log in first, and then try another time to click on the email token.");
                return;
            }
            if (userKey == null || userKey.isEmpty()) {
                showDisplayWarning(
                        ar,
                        "Weaver can not understand that email verfication link.  It appears to be improperly formed.  Please request a new email verification link.");
                return;
            }
            Cognoscenti cog = ar.getCogInstance();
    ...
    public void sendEmail(
        ...
        try {
            ...
            AddressListEntry toUser =
                    AddressListEntry.findOrCreate(destinationUser.getPreferredEmail());
            ...
            File templateFile =
                    ar.getCogInstance().getConfig().getFileFromRoot("email/DirectEmail.chtml");
    ...
    public void updateFacilitatorInfo(
            @PathVariable String userKey,
            HttpServletRequest request,
            HttpServletResponse response) {
        AuthRequest ar = AuthRequest.getOrCreate(request, response);
    ...
    /** This method is primarily for uploading a temporary file for the use as a user icon. */
    @RequestMapping(value = "/{userKey}/TempFile/{tempName}", method = RequestMethod.PUT)
    public void doPut(
            @PathVariable String userKey,
            @PathVariable String tempName,

...
public class BaseController {
    ...
    /**
     * Call this from the initialization classes in order to configure how the spring controller
     * classes operate.
     */
    ...
    /**
     * Logs the exception in the tracker, and generates a 'nice' display of an exception. This can
     * be used when the exception is predictable, such a parsing a URL and not finding the data that
     * it should display. You should also be aware of the problems of displaying an exception when
     * the system is truly breaking -- the support necessary to create the nice display might not be
     * functioning well enough to display anything.
     */
    private void displayException(AuthRequest ar, Exception extd) {
        try {
            long exceptionNO =
                    ar.logException("SPRING MAPPED EXCEPTION: " + ar.getCompleteURL(), extd);
    ...
    /**
     * This is a set of checks that results in different views depending on the state of the user.
     * Particularly: must be logged in, must have a name, must have an email address, and must be a
     * member of the page, so the page has to be set as well.
     *
    ...
    protected static boolean warnNoAccess(AuthRequest ar) throws Exception {
        ...
        if (!ar.canAccessWorkspace()) {
            if (ar.ngp instanceof NGBook) {
                ...
                showDisplayWarningF(
                        ar,
                        "In order to see this section, (%s) needs to be a workspace member.",
                        ar.getBestUserId());
            }
            return true;
        }
    ...
    /**
     * Checks that the project is OK to be updated. Should be used for forms that prompt for
     * information to update the workspace. In that case a frozen (or deleted) workspace should not
     * prompt for that update.
     *
     * @throws Exception
     */
    protected static boolean warnFrozenOrNotMember(AuthRequest ar, NGContainer ngc)
            throws Exception {
        if (warnNoAccess(ar)) {
            return true;
        }
        ...
        try {
            ...
            if (!jspFile.exists()) {
                ...
                if (!ar.isLoggedIn()) {
                    ...
                    showDisplayWarningF(
                            ar, "Unable to access resource %s / %s", accessLevel, jspName);
                }
            } else {
    ...
    public static NGContainer registerSiteOrProject(AuthRequest ar, String siteId, String pageId)
            throws Exception {
    ...
    /**
     * This is a convenience function for all handlers that have the account and project in the URL.
     *
     * <p>(1) This will validate those values. (2) Read the project. (3) Sets the access level to
     * the page (4) Set the header type to be project (5) Throws and exception if anything is wrong.
     *
     * <p>Will ALSO set two request attributes needed by the JSP files.
     */
    public static NGWorkspace registerWorkspaceRequired(
            AuthRequest ar, String siteId, String pageId) throws Exception {
    ...
    public static void showJSPDepending(
            AuthRequest ar, NGWorkspace ngw, String jspName, boolean specialAccess)
            throws Exception {
        try {
            if (warnSiteMoved(ar)) {
                return;
    ...
    public static void showJSPAnonymous(
            AuthRequest ar, String siteId, String pageId, String jspName) throws Exception {
        try {
    ...
    public static void showJSPLoggedIn(AuthRequest ar, String siteId, String pageId, String jspName)
            throws Exception {
        try {
            registerSiteOrProject(ar, siteId, pageId);
            if (warnNotLoggedIn(ar)) {
     ...
     */
    public static void showJSPSiteLiberal(AuthRequest ar, String siteId, String jspName)
            throws Exception {
        try {
    ...
    public static void showJSPMembers(
            AuthRequest ar, String siteId, String pageId, String jspName) {
        try {
            registerSiteOrProject(ar, siteId, pageId);
            if (warnNoAccess(ar)) {
                return;
    ...
    public static AuthRequest getLoggedInAuthRequest(
            HttpServletRequest request, HttpServletResponse response, String assertLoggedInMsgKey)
            throws Exception {
    ...
    protected void redirectBrowser(AuthRequest ar, String pageURL, Properties props)
            throws Exception {
        char joinChar = '?';
    ...
    protected void streamException(Exception e, AuthRequest ar) {
        try {
            ...
            System.out.println(
                    "DOUBLE EXCEPTION (BaseController) tid="
                            + Thread.currentThread().threadId()
                            + ", "
                            + eeeee.toString());
        }
    ...
    /**
     * This is a testing tool. It is simply a delay. In normal production operation this delay
     * should be zero in order to respond as quickly as possible. But in order to test latency
     * situations, this can be a one to three to ten second delay to slow down the response time.
     */
    ...
    /**
     * Call this to properly send the JSON back to the client. This should be the LAST thing of a
     * JSON call
     */
    ...
    /**
     * Should probably NEVER be sending an array, but there are some cases where it was done and so
     * this method supports it while cleaning them out.
     */
    ...
    protected static void saveAndReleaseLock(NGWorkspace ngw, AuthRequest ar, String msg)
            throws Exception {
        ngw.saveFile(ar, msg);
        NGPageIndex.clearLocksHeldByThisThread();
    }

    public void streamJSPUserLogged2(

...
/**
 * This class will handle all requests managing process/tasks. Currently this is handling only
 * requests that are coming to create a new task, update/modify task and also reassign the task to
 * other member.
 */
@Controller
public class ProjectGoalController extends BaseController {
    ...
    /**
     * Access right on this are a little complicated. These elements 1. are you logged in 2. if
     * logged in, are you a member of the workspace people assigned to a task are considered members
     * while the task is open 3. do you have a magic number for the task in order access it
     * anonymously.
     *
     * <p>Situations:
     *
     * <p>1. login-yes, member-yes, magic number-NA 2. login-no, member-NA, magic number-no, show
     * the login error page 3. login-no, member-NA, magic number-yes, show the special anon page 4.
     * login-yes, member-no, magic number-no, show not a member error 5. login-yes, member-no, magic
     * number-yes, show the special anon page
     */
    ...
    public void displayTask(
        ...
        try {
            ...
            if (goal == null) {
                showDisplayWarning(
                        ar,
                        "Can not find an action item with the id  "
                                + taskId
    ...
    public void updateGoal(
        ...
        try {
            ...
            StringBuilder inventedComment =
                    new StringBuilder(goalInfo.optString("newAccomplishment"));
            ...
            if (hasChanged) {
                ...
                HistoryRecord.createHistoryRecord(
                        ngw, gr.getId(), HistoryRecord.CONTEXT_TYPE_TASK, eventType, ar, comments);
            }
        ...
        try {
            ...
            if (!canAccessGoal) {
                ar.assertAccessWorkspace(
                        "Need to have access to workspace to see action item history");
            }
    ...
    public void updateDecision(
        ...
        try {
            ...
            if (isNew) {
                ...
                if (!decisionInfo.has("timestamp")
                        || decisionInfo.optLong("timestamp", -1) < 100000000) {
            ...
            HistoryRecord.createHistoryRecord(
                    ngw,
                    Integer.toString(dr.getNumber()),
                    HistoryRecord.CONTEXT_TYPE_DECISION,
                    eventType,
                    ar,
                    "update decision");
    ...
    public void moveTaskArea(
        ...
        try {
            ...
            ar.setPageAccessLevels(ngw);
            ar.assertNotFrozen(ngw);
            ar.assertUpdateWorkspace("Update access needed to change the Task Areas.");
    ...
    @RequestMapping(
            value = "/{siteId}/{pageId}/ActionItem{actId}Due.ics",
    ...
    public void moveActionItem(
        ...
        try {
            ...
            oldGoal.setDescription(
                    "This task was moved to another workspace: " + thisWS.getFullName());
            fromWS.saveModifiedWorkspace(
                    ar, "Action Item moved to another workspace: " + thisWS.getFullName());

...
public class MeetingNotesCache {
    ...
    private class NoteHolder {
        ...
        private void calculateMemberList(AuthRequest ar) throws Exception {
            NGWorkspace ngw =
                    ar.getCogInstance()
                            .getWSBySiteAndKeyOrFail(siteKey, workspaceKey)
                            .getWorkspace();
            ...
            for (String part : meet.getParticipants()) {
                AddressListEntry.addIfNotPresent(
                        targetPlayers, AddressListEntry.findOrCreate(part));
            }
    ...
    public boolean canAcccessMeeting(
            String site, String workspace, AuthRequest ar, String meetingId) throws Exception {
        String key = site + "|" + workspace + "|" + meetingId;
        NoteHolder nh = cache.get(key);
        if (nh == null) {
            // this gets a lock and that can block....
            NGWorkspace ngw =
                    ar.getCogInstance().getWSBySiteAndKeyOrFail(site, workspace).getWorkspace();
    ...
    public JSONObject getOrCacheNotes(
            String site, String workspace, AuthRequest ar, String meetingId) throws Exception {
        String key = site + "|" + workspace + "|" + meetingId;
        NoteHolder nh = cache.get(key);
        if (nh != null) {
    ...
    public JSONObject getOrCacheFull(
            String site, String workspace, AuthRequest ar, String meetingId) throws Exception {
        String key = site + "|" + workspace + "|" + meetingId;
        NoteHolder nh = cache.get(key);
        if (nh != null) {
    ...
    public JSONObject updateCacheNotes(NGWorkspace ngw, AuthRequest ar, String meetingId)
            throws Exception {
        NoteHolder nh = cacheMeeting(ngw, ar, meetingId);
        return nh.notesObject;
    }

    public JSONObject updateCacheFull(NGWorkspace ngw, AuthRequest ar, String meetingId)
            throws Exception {
        NoteHolder nh = cacheMeeting(ngw, ar, meetingId);
        return nh.fullObject;
    }

    private NoteHolder cacheMeeting(NGWorkspace ngw, AuthRequest ar, String meetingId)
            throws Exception {

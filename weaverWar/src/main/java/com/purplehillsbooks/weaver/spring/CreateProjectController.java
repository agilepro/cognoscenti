...
/**
 * This class will handle all requests that are coming to create a new Workspace. Currently this is
 * handling only requests that are coming to create a new project from scratch. Later this can be
 * extended to edit a project or to create a project from Template.
 */
@Controller
public class CreateProjectController extends BaseController {
    ...
    public void createWorkspaceAPI(
        ...
        try {
            ...
            site.assertSiteExecutive(
                    ar.getUserProfile(),
                    "Must be an executive of a site to create a new workspace");

            JSONObject newConfig = getPostedObject(ar);
            String workspaceName = newConfig.getString("newName");

            // first, if given a template, check to make sure it exists else error before creating
            // workspace
            ...
            if (newConfig.has("template")) {
                ...
                if (template.length() > 0) {
                    templateWorkspace =
                            ar.getCogInstance().getWSByCombinedKeyOrFail(template).getWorkspace();
                }
            }
            ...
            if (newConfig.has("members")) {
                ...
                if (members != null) {
                    ...
                    for (int i = 0; i < members.length(); i++) {
                        ...
                        memberRole.addPlayerIfNotPresent(
                                AddressListEntry.findOrCreate(memberAddress));
                    }
                }
            }
    ...
    /**
     * Takes a nice display name and turns it into a nice URL value.
     *
     * <p>The input: "My favorite, usual Hangout!" produces: "my-favorite-usual-hangout"
     *
     * <p>All the letters are lower cased. Only A-Z is supported with numerals any gaps of other
     * characters of any kind replaced with hyphen hyphen avoided on either end
     */
    ...
    /**
     * Creates a workspace, but does not save it. Caller must save the workspace ... otherwise bad
     * things could happen.
     */

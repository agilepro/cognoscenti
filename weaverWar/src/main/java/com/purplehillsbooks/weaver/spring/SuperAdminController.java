...
public class SuperAdminController extends BaseController {
    ...
    @RequestMapping(value = "/su/lookUpUser.json", method = RequestMethod.POST)
    public void lookUpUser(HttpServletRequest request, HttpServletResponse response) {
        AuthRequest ar = AuthRequest.getOrCreate(request, response);
        try {
            ar.assertLoggedIn("Must be logged in to look up a user.");
            JSONObject requestInfo = getPostedObject(ar);
            String email = requestInfo.getString("uid");

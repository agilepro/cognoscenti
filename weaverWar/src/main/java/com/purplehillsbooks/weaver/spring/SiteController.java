...
public class SiteController extends BaseController {
    ...
    public void accountCreateProject(
            @PathVariable String siteId, HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        AuthRequest ar = AuthRequest.getOrCreate(request, response);
    ...
    /**
     * This is the Site Administrator version of adding user to role. It receives a workspace, role,
     * and user to either add or remove that user
     */
                ...
                if (isPresent) {
                    System.out.println(
                            "ROLE: workspace ("
                                    ...
                                    + ") role ("
                                    + roleName
                                    + ") attempt to add when user is already present: "
                                    + userId);
                } else {
                ...
                if (isPresent) {
                    ...
                    System.out.println(
                            "ROLE: workspace ("
                                    ...
                                    + ") role ("
                                    + roleName
                                    + ") attempt to remove when user is not there: "
                                    + userId);
                }
            }
            if (madeChange) {
    ...
    public void siteMail(
        ...
        try {
            ...
            // save, but don't update the recently changed date, site mail does not represent real
            // site
            // activity
            ngb.save();
    ...
    public void AllPeople(
            HttpServletRequest request, HttpServletResponse response, @PathVariable String siteId)
            throws Exception {

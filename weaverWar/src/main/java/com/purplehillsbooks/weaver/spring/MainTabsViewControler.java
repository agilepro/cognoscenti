...
public class MainTabsViewControler extends BaseController {
        ...
        try {
            ...
            if (ar.isLoggedIn()) {
                response.sendRedirect(
                        ar.retPath + "v/" + ar.getUserProfile().getKey() + "/UserHome.htm");
                return;
            }
    ...
    /** This removes the logged in user from a specified role p == workspace id role == role name */
    ...
    public void searchPublicNotesJSON(
        ...
        try {
            ...
            if (searchFilter.length() > 0) {
                searchResults =
                        ar.getCogInstance()
                                .performSearch(ar, searchFilter, searchProject, siteId, pageId);
            } else {

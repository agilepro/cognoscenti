...
/**
 * The purpose of this class is to wrap the Spring DispatcherServlet object in a way that convert
 * the HTTPResponse parameter into a HttpServletResponseWithoutBug object, in order to avoid
 * problems with getting the output stream more than one time.
 *
 * <p>See HttpServletResponseWithoutBug for more details.
 */
...
public class SpringServletWrapper extends HttpServlet {
    ...
    /**
     * According to the java doc, all the http requests come through this method and later get
     * redirected to the other requests by type: GET, PUT, etc. All we want to do is to wrap the
     * response object, and this should do it for all request types.
     */
    protected void service(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            System.out.println(
                    "[Web URL: " + requestAddr + "] tid=" + tid + " start=" + (startTime % 10000));
            ...
            System.out.println(
                    "     exception "
                            ...
                            + (endTime % 10000));
            ar.logException(
                    "Unable to handle web request to URL (" + requestAddr + ") tid=" + tid, e);
    ...
    public void destroy() {
        System.out.println(
                "STOP - SpringServletWrapper has been called to DESTROY "
    ...
    /**
     * Can generate any page in the system. Page is generated to the Writer in the AuthRequest
     * object, and according the URL parameters in the AuthRequest object.
     *
     * <p>For static site generation and testing, create a new nested auth request so the original
     * request will not be disturbed. The nested auth request object takes a relative URL to that
     * page desired. It also takes a Writer to write the output to.
     */

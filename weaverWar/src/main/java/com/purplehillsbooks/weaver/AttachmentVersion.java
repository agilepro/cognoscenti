...
/**
 * A workspace folder file versioning system just represents multiple files in a file system. All
 * version are stored in a subfolder named ".cog". and are named as the internal ID and appending
 * the version number to that ID. For example the following might exist in the file system:
 *
 * <p>great-cars/.cog/9987-1.xls great-cars/.cog/9987-2.xls great-cars/.cog/9987-3.xls
 * great-cars/.cog/9987-4.xls great-cars/.cog/9987-5.xls
 *
 * <p>In this case, the workspace folder (container) is "great-cars" The attachment id is "9987"
 * which is unique within the container. The hyphen number at the end is the version number of the
 * attachment.
 *
 * <p>The drawback of the workspace versioning system is obvious: all versions of all files are in
 * the file system at the same time, taking up space. Also, the files are all stored local to one
 * system. The advantage, however, is that it does not depend upon complex code to unpack or
 * otherwise manipulate things to access the versions. Simply find the version by name, and stream
 * it out. Very fast.
 *
 * <p>Disks are cheap, and if your documents are mostly binary, then there is little need to use a
 * differencing algorithm to store more efficiently.
 *
 * <p>The current version file is no longer duplicated in the workspace folder. Used to be these for
 * direct editing, but we only support servers now.
 *
 * <p>Date and time of the version is taken directly from the file system date.
 *
 * <p>There is no record of who made the change, and other helpful metadata that you would get in a
 * real versioning system. Might think about adding another structure in the .cog folder to track
 * that.
 *
 * <p>The attachment can be in these states: (FILE, or URL)
 *
 * <p>"FILE" is the normal situation, there is a file in the main folder, and there is at least one
 * version in the COG folder that exactly equals it.
 *
 * <p>"URL" is data only, there are no files on the disk.
 *
 * <p>"EXTRA" - deprecated, not used
 *
 * <p>"GONE" - deprecated, not used
 *
 * <p>"GHOST" - deprecated, not used
 *
 * <p>"DELETED" - this state is when the user deleted the file. There should still be files in the
 * COG folder.
 *
 * <p>"MODIFIED" - deprecated, not used
 */
public class AttachmentVersion {
    ...
    /**
     * This is the static method that will search the file system for all of the attachments for a
     * given container and attachment id. Keeping this code here in the class keeps all the Simple
    ...
    /**
     * This static method does the right thing for project versioning system to get a new version
     * file. Calculates the name of the new file, and it streams the entire contents to the file in
     * the attachment folder. This method is synchronized so that only one thread will be creating a
     * new version at a time, and there is no confusion about what version a file is.
     */
    public static synchronized AttachmentVersion getNewWorkspaceVersion(
            File wsFolder, AttachmentRecord att, InputStream contents) throws Exception {
        String attachName = att.getNiceName();
        String attachmentId = att.getId();
        File cogFolder = new File(wsFolder, ".cog");
        if (!cogFolder.exists()) {
        ...
        // First, lets copy the new contents here, so that there is no blocking while in the
        // synchronized
        // block.  Create a local file in the attachments folds, and copy the file there, so that
        // later the rename will be very fast.
        File tempCogFile =
                new File(cogFolder, "~tmpV-" + attachmentId + "-" + unique() + fileExtension);
    ...
    public String getLink() {
        if (attachment.isURL()) {
            ...
            return "a/"
                    + SectionUtil.encodeURLData(attachment.getNiceName())
                    + "?version="
                    + this.getNumber();
        }
    }

...
public class DailyDigest {
    ...
    public static void sendDailyDigest(AuthRequest arx, Cognoscenti cog) throws Exception {
        ...
        File dailyDigestFile =
                new File(cog.getConfig().getUserFolderOrFail(), "DailyDigestLog.json");

        try {
            NGPageIndex.assertNoLocksOnThread();
            long lastNotificationSentTime =
                    arx.getSuperAdminLogFile().getLastNotificationSentTime();
            ...
            for (UserProfile up : arx.getCogInstance().getUserManager().getAllUserProfiles()) {
                ...
                if (up.getLastLogin() <= 0) {
                    ...
                    debugWriter.write(
                            "\n<li>No email because user inactive never logged in: "
                                    + up.getUniversalId()
                                    + "</li>");
                ...
                if (up.getLastLogin() < oneYearAgo) {
                    ...
                    debugWriter.write(
                            "\n<li>No email because user inactive for a year: "
                                    + up.getUniversalId()
                                    + "</li>");
            ...
            SuperAdminLogFile.getInstance(cog)
                    .setLastNotificationSentTime(processingStartTime, debugStuff.toString());
    ...
    private static void handleOneUser(
        ...
        try {
            ...
            long earliestPossible =
                    System.currentTimeMillis()
                            - (up.getNotificationPeriod() * 2L * 24 * 60 * 60 * 1000);
            ...
            for (String noteKey : up.getNotificationList()) {
                ...
                List<HistoryRecord> histRecs =
                        ngw.getHistoryRange(historyStartTime, processingStartTime);
                if (histRecs.size() == 0) {
            ...
            OptOutAddr ooa = new OptOutAddr(AddressListEntry.parseCombinedAddress(realAddress));
            ...
            SectionUtil.nicePrintDateAndTime(clone.w, historyStartTime);
            clone.write("</b> and ending <b>");
            SectionUtil.nicePrintDateAndTime(clone.w, processingStartTime);
            clone.write("</b></p>\n");
...

            {
                ...
                if (containers.size() > 0) {
                    clone.write(
                            "<div style=\"margin-top:15px;margin-bottom:20px;\"><span style=\"font-size:24px;font-weight:bold;\">Workspace Updates</span>&nbsp;&nbsp;&nbsp;");
                    NGPageIndex.clearLocksHeldByThisThread();
                    numberOfUpdates +=
                            constructDailyDigestEmail(
                                    clone, containers, historyStartTime, processingStartTime);
                    clone.write("</div>");
                }
            ...
            if ((numberOfUpdates > 0) || numTasks > 0) {
                String thisSubj =
                        "Daily Digest - " + numberOfUpdates + " updates, " + numTasks + " tasks.";
            ...
            arx.logException("Error while sending daily update message", e);
            ...
            try {
                ...
                System.out.println(
                        "Exception while reporting an exception sending daily digest email to user: "
                                + eeeeee);
            }
        }
    }
    ...
    public static int constructDailyDigestEmail(
            AuthRequest clone,
            List<NGPageIndex> containers,
            long historyRangeStart,
            long historyRangeEnd)
            throws Exception {
        NGPageIndex.assertNoLocksOnThread();
        ...
        for (NGPageIndex ngpi : containers) {
            ...
            try {
                NGWorkspace ngw = ngpi.getWorkspace();
                List<HistoryRecord> histRecs =
                        ngw.getHistoryRange(historyRangeStart, historyRangeEnd);
                if (histRecs.size() == 0) {
                    // skip this if there is nothing to show
                    continue;
                }
                String url = clone.retPath + clone.getDefaultURL(ngw);
                ...
                for (HistoryRecord history : histRecs) {
                    ...
                    SectionUtil.nicePrintTime(clone.w, history.getTimeStamp(), clone.nowTime);
    ...
    /**
     * this is a reminder to super admins that there are sites waiting to be granted or not. Not
     * sure this is needed.
     *
     * @param clone
     * @param delayedSites
     * @throws Exception
     */
    private static void writeDelayedSiteList(AuthRequest clone, List<SiteRequest> delayedSites)
            throws Exception {
    ...
    private static int formatActionItems(AuthRequest ar, UserProfile up, UserCache userCache)
            throws Exception {
        int taskNum = 0;
        ...
        ar.write(
                "<div style=\"margin-top:25px;margin-bottom:5px;\"><span style=\"font-size:24px;font-weight:bold;\">Task Updates</span>&nbsp;&nbsp;&nbsp;");
        ...
        for (JSONObject actionItemObj : actionItems.getJSONObjectList()) {
            ...
            } else if (taskState == BaseRecord.STATE_ACCEPTED
                    || taskState == BaseRecord.STATE_OFFERED
                    || taskState == BaseRecord.STATE_WAITING) {
                // allow this
            } else {
                continue;
            }
    ...
    /**
     * Writes a URL of the task details page for a given task along with the magic number and user
     * key for anonymous access.
     */
    private static void writeGoalLinkUrl(
            AuthRequest ar, JSONObject actionItemObj, String userKey, UserCache cache)
            throws Exception {

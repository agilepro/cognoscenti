...
public class TemplateProviderWeaver extends TemplateProvider {
    ...
    /**
     * no idea what this does, however some of the sample classes return 'include' so doing that
     * here
     */
    @Override
    ...
    public boolean provides(String templateName) {
        try {
            // System.out.println("TemplateProviderWeaver.provides request for
        ...
        // System.out.println("TemplateProviderWeaver.provides returned FALSE for
        // template="+templateName);
        return false;
    }
    ...
    /**
     * Apparently this method is to read the template file and return the contents as a string. It
     * is apparently given the full file name "MyTemplate.chtml"
     */
    @Override
    public String loadContainerDoc(String templateName) throws IOException {
        try {
            // System.out.println("TemplateProviderWeaver.loadContainerDoc called for
            // template="+templateName);
            if (!templateName.endsWith(".chtml")) {
                throw WeaverException.newBasic(
                        "Something is wrong, the file should end with .chtml");
            }
            File templateFile = ar.findChunkTemplate(templateName);
            // System.out.println("TemplateProviderWeaver found
            ...
            System.out.println(
                    "TemplateProviderWeaver.loadContainerDoc EXCEPTION for " + templateName);
            e.printStackTrace(System.out);
            throw new IOException(
                    "Error in TemplateProviderWeaver reading of template: " + templateName, e);
        }
    }
}

 ...
 * username and token, passed in the header of the request, in order to access the resource that the
 * url points at.
 *
 * <p>The members of this class are public because there is no special magic behind them. This class
 * is a bundle of three values: the url, the username, and the token. These are three entirely
 * independent values that can be changed at any time
 */
public class LicensedURL {
    ...
    public static String addLicense(String url, String token) throws Exception {
    ...
    /**
     * When making a request to the subprocess, you need to pass a username and a token (password).
     * is a format which includes the username and token directly in the URL. This format works in
     * some versions of browsers, but not all, and in fact may not be part of the standard. This
     * combined version is simply a wy to pack all three values togehter so that a user can copy and
     * paste all three values with a single operation.
     *
     * <p>The format is http://username:password@host:port/app/.....(rest of the url)
     *
     * <p>This method composes the combined form from the three individual values.
     */
    ...
    /**
     * Parses the license token out of the URL combined representation and returns a valid Licensed
     * URL object back.
     */
    public static LicensedURL parseCombinedRepresentation(String newVal) throws Exception {
    ...
    /**
     * This method allows you to set the URL, username, and token with a single call using the form
     * where all three values are combined into a single string value.
     *
     * <p>returns an array of three strings: url, username, token
     */
    public static LicensedURL parseCombinedRepresentationOLD(String newVal) throws Exception {
        StringBuilder res = new StringBuilder();
        int pos = 0;

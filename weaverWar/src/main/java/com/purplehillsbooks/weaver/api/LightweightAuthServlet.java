 ...
 * both POST method, and both receiving and returning JSON structures. A JavaScript client can use
 * these method to authenticate the user to this server.
 *
 * <p><server>/<app>/auth/getChallenge
 *
 * <p>This receives a request for a challenge value, generates a challenge value, associates the
 * challenge with the current session, and returns the challenge in a JSON file.
 *
 * <p><server>/<app>/auth/verifyToken
 *
 * <p>This receives a JSON with the challenge, the token from the auth provider. This will make a
 * call to the auth provider, and if it gets an acceptable response (verified) then it records the
 * user information in the session, and returns a value saying that the user is logged in.
 */
...
public class LightweightAuthServlet extends jakarta.servlet.http.HttpServlet {
    ...
    private void setHeadersForRediculousBrowserAuthRequirements(
        ...
        try {
            ...
            if (pathInfo.startsWith("/logout")) {
                ...
                if (objIn.getString("token").length() == 0) {
                    throw new Exception(
                            "'verifyToken' needs to have a 'token' member of the passed JSON.");
                }

                if (!chg1.equals(chg2)) {
                    aStat.clearChallenge();
                    throw new Exception(
                            "Got a request to verify a token and challenge that is not that which was given out.  Authentication transaction aborted.");
                }
                ...
                if (trusterProviderUrl == null || trusterProviderUrl.length() == 0) {
                    throw new Exception(
                            "the LightweightAuthServlet has not been initialized with the address of the provider");
                }
    ...
    public static JSONObject postToTrustedProvider(String postFix, JSONObject objIn)
            throws Exception {

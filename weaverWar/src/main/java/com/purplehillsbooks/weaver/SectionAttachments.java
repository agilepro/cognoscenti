...
public class SectionAttachments extends SectionUtil implements SectionFormat {
    ...
    /**
     * READ CERFULLY: This method is placed here to perform any schema migration or any clean up
     * tasks that need to be done on a document as it is read. This method is called when the page
     * is read into memory. The migration will always update the document in memory, so that IF the
     * document is written out, it will always be written in the latest form.
     *
     * <p>Each piece of schema migration code must be dated as of the date it was introduced, so
     * that when all documents have finally been updated, that code can be removed as it is no
     * longer needed. Generally we will leave such code in place for a period of two years.
     *
     * <p>Care must be take so that new additions of the schema are not confused with older versions
     * of the schema.
     */
    public static void assureSchemaMigration(NGSection sec, NGWorkspace ngw) throws Exception {
        ...
        List<AttachmentRecord> oldAtts =
                oldAttachmentsContainer.getChildren("attachment", AttachmentRecord.class);
    ...
    /**
     * pass ar=null if you do not want any history records created recording that the document was
     * removed.
     */
    public static void removeAttachments(
            AuthRequest ar, NGSection ngs, List<String> fileIdsToBeRemoved) throws Exception {
    ...
    /**
     * This is a method to find a file, and output the file as a stream of bytes to the request
     * output stream.
     */
    ...
    /**
     * Returns a stream from which the contents of the file can be read. Be sure to close the stream
     * when you are done with it.
     *
     * <p>Specified version is returns, except if specified version does not exist, and then the
     * latest version is returned. Throws exceptions if no version or no contents can be found.
     */
    public static AttachmentVersion getVersionOrLatest(
            NGWorkspace ngw, String fileName, int version) throws Exception {
        try {

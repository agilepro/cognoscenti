...
public class UserCache {
    ...
    public void refreshCache(Cognoscenti cog) throws Exception {
        ...
        for (NGPageIndex ngpi : cog.getWorkspacesUserIsIn(up)) {
            ...
            for (GoalRecord actionItem : ngw.getAllGoals()) {
                ...
                magicNumbers.put(
                        actionItem.getId(), AccessControl.getAccessGoalParams(ngw, actionItem));
            }
            for (TopicRecord aTopic : ngw.getAllDiscussionTopics()) {
                ...
                if (aTopic.isDraftNote()
                        && (up.hasAnyId(aTopic.getModUser().getUniversalId())
                                || up.hasAnyId(aTopic.getOwner()))) {
                    ...
                    magicNumbers.put(
                            aTopic.getId(), AccessControl.getAccessTopicParams(ngw, aTopic));
                }
    ...
    private void addPollIfNoResponse(
            JSONArray proposalList,
            JSONArray openRounds,
            CommentRecord cr,
            UserProfile up,
            NGWorkspace aPage,
        ...
        if (cr.getState() == CommentRecord.COMMENT_STATE_OPEN) {
            if (up.equals(cr.getUser())) {
                ...
                // If user created this round, then remember that ... because it is still open or
                // draft
    ...
    private void addRecordToList(JSONArray list, CommentRecord cr, NGWorkspace aPage)
            throws Exception {
    ...
    private void addMeetingToList(
            JSONArray list, MeetingRecord meet, NGWorkspace aPage, String address)
            throws Exception {
    ...
    /**
     * Get a list of the comments (proposals and rounds) that still open but not yet closed, whether
     * overdue or not.
     */
    ...
    public String verifyEmailAddressAttempt(UserProfile user, String token) throws Exception {
        ...
        String resultMsg =
                String.format(
                        "Verification link token is not valid for (%s).  Tokens must be used within 7 days of being sent by the user that sent it.  If you still need to add the email address to your profile, please request a new confirmation message.",
                        user.getName());
        ...
        for (int i = 0; i < allAttempts.length(); i++) {
            ...
            if (token.equals(oneAttempt.getString("token"))) {
                ...
                if (user.hasAnyId(email)) {
                    resultMsg =
                            String.format(
                                    "The email address (%s) is already part of your user profile, looks like you have already used this verification link.",
                                    email);
                } else {
                    user.addId(email);
                    ...
                    resultMsg =
                            String.format(
                                    "Success!  You have now confirmed the email address (%s) and it has been added to your user profile.",
                                    email);
                }
            }

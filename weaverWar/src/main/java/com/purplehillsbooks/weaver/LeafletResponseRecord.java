    ...
    public LeafletResponseRecord(Document definingDoc, Element definingElement, DOMFace lr) {
        ...
        if (user == null || user.length() == 0) {
            throw new RuntimeException(
                    "A topic response tag MUST have an " + "attribute 'user' with a valid value");
        }
    }

    /**
     * The user is the key to this record. There is no way to change the key (no way to set it). It
     * must be created with a particular key. You can read it with this method.
     */
